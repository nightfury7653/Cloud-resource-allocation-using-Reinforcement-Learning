#!/usr/bin/env python3
"""
DDQN Training Script

Train a DDQN agent on the realistic cloud resource allocation environment.

Usage:
    python scripts/train_ddqn.py                          # Train with default settings
    python scripts/train_ddqn.py --episodes 500           # Train for 500 episodes
    python scripts/train_ddqn.py --resume checkpoint.pt   # Resume from checkpoint
    python scripts/train_ddqn.py --eval-only              # Evaluation only
"""
...
class DDQNTrainer:
    ...
    def __init__(self, 
        ...
        # Setup logging
        self.log_dir = log_dir or f"results/logs/ddqn/{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.writer = None
...
def main():
    ...
    # Load config
    import yaml
    config_path = args.config or Path(__file__).parent.parent / 'config' / 'ddqn_config.yaml'
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)
    
    # Create trainer
    trainer = DDQNTrainer(env, agent, config)
